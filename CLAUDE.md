...
- **Two libraries are vendored in [lib/](lib/) and committed** because they carry local patches; neither may reappear in `lib_deps` (the registry copy would shadow the patched one): `ESP32-BLE-Gamepad` (patch: see bullet below; only its NimBLE dependency is in `lib_deps`, pinned `^2.5`) and `ArduinoWebsockets` 0.5.4 (**large-frame patch** in [lib/ArduinoWebsockets/src/websockets_endpoint.cpp](lib/ArduinoWebsockets/src/websockets_endpoint.cpp): the stock code treats a momentarily empty TCP buffer as end-of-frame and silently truncates anything bigger than the lwIP window — ~100 KB `getLogEntries`/`getSystem` responses arrived empty. The patch waits bounded (3 s) for the stream, skips the wait for zero-payload frames (pongs), and drops truncated frames instead of delivering zero-padded garbage).
