...
- **`ESP32-BLE-Gamepad` needs a vendored patch:** its `BleNUS.h`/`BleNUS.cpp` rely on `Arduino.h` being included transitively (true under NimBLE 1.x, *not* under the NimBLE 2.x pulled in by the new core). A manual `#include <Arduino.h>` was added to `.pio/libdeps/default/ESP32-BLE-Gamepad/BleNUS.h`. **This lives in `.pio/` and is lost whenever libs are reinstalled or `.pio` is deleted** — re-apply it if `delay`/`Serial`/`ltoa`/`dtostrf` "not declared" errors reappear in BleNUS.
