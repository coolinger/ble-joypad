...
// LOCAL PATCH (BLEJoy, 2026-07-07): large frames (e.g. 100 KB getLogEntries
// responses) span many TCP segments; the stock code treats every momentary
// empty receive buffer as end-of-frame and silently truncates. We wait
// (bounded) for the stream to continue instead. Arduino-only (millis/delay);
// this vendored copy is used on ESP32 exclusively.
#ifdef ARDUINO
#include <Arduino.h>
#endif
...
namespace internals {
    ...
    // LOCAL PATCH: bounded wait for payload bytes. A momentary empty TCP
    // buffer is NORMAL mid-frame (lwIP RX window ~5.7 KB, frames up to
    // 100+ KB); only a real stall/disconnect ends the read.
    static bool waitForData(network::TcpClient& socket, uint32_t timeoutMs) {
#ifdef ARDUINO
        uint32_t start = millis();
        while (!socket.available()) {
            if (!socket.available() && (millis() - start) > timeoutMs) return false;
            delay(1);
        }
        return true;
#else
    ...
    WebsocketsFrame WebsocketsEndpoint::_recv() {
        ...
        // LOCAL PATCH: the rest of the frame may still be in flight - wait
        ...
        if (header.mask) {
            ...
            // LOCAL PATCH: bounded wait for the payload behind the mask key.
            if(payloadLength > 0 && !waitForData(*this->_client, 3000)) return WebsocketsFrame();
        }
